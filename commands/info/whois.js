...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (target.presence.status === "dnd")
      target.presence.status = "Do Not Disturb <:dboatsDnd:746681406863966228>";
    if (target.presence.status === "idle")
      target.presence.status = "Idle <:9089_discord_idle:746681289654141041>";
    if (target.presence.status === "online")
      target.presence.status = "Online <:bfdonline:746681365570912336>";
    if (target.presence.status === "offline")
      target.presence.status = "Offline <:invisible:746682501501550622>";
